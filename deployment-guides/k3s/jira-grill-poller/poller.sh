...
# bot 觸發目標 bot（GRILL_TARGET_BOT，預設 Rick）。細節見
# docs/superpowers/specs/2026-08-25-jira-grill-poller-design.md 與
# docs/superpowers/specs/2026-09-08-genie-jira-grill-routing-design.md。
...
# GRILL_TARGET_BOT：這一輪 grill-me 要 mention 哪個 bot，預設 rick（沿用
# 現行行為）。之後想切換給 genie 接手，只改這個環境變數，不用改這支
# script。見 docs/superpowers/specs/2026-09-08-genie-jira-grill-routing-design.md。
: "${GRILL_TARGET_BOT:=rick}"
case "$GRILL_TARGET_BOT" in
  rick)
    : "${RICK_DISCORD_USER_ID:?missing RICK_DISCORD_USER_ID}"
    TARGET_BOT_ID="$RICK_DISCORD_USER_ID"
    TARGET_BOT_NAME="Rick"
    ;;
  genie)
    : "${GENIE_DISCORD_USER_ID:?missing GENIE_DISCORD_USER_ID}"
    TARGET_BOT_ID="$GENIE_DISCORD_USER_ID"
    TARGET_BOT_NAME="Genie"
    ;;
  *)
    echo "ERROR: 不支援的 GRILL_TARGET_BOT=${GRILL_TARGET_BOT}（只支援 rick/genie）" >&2
    exit 1
    ;;
esac
...
# POST /rest/api/3/search/jql（官方遷移指引指定的替代端點）。回應格式
# 改用 nextPageToken/isLast 分頁，這裡只取第一頁（maxResults 50）——
# 本 poller 的查詢範圍窄（單一 project 白名單 + 特定 label），單頁
# 已足夠，不做分頁跟進。
jira_search() {
  SEARCH_BODY=$(node -e '
...
trigger_discord() {
  ...
  BODY=$(node -e '
    ...
    process.stdout.write(JSON.stringify({
      content: "<@" + targetId + "> 執行 jira-grill skill，參數：ticket " + ticket
    }));
  ' "$TARGET_BOT_ID" "$TICKET_ID")
...
if [ -n "$ACTIVE_TICKETS" ]; then
  echo "$ACTIVE_TICKETS" | while IFS= read -r TICKET_ID; do
    ...
    NEEDS_TRIGGER=$(printf '%s' "$RESPONSE" | node -e '
      ...
      const signature = "— By " + process.argv[2] + " (jira-grill)";
      console.log(comments[0].body.includes(signature) ? "skip" : "trigger");
    ' "$TICKET_ID" "$TARGET_BOT_NAME")
