...
- 若該 repo 的 `CLAUDE.md`/`AGENTS.md` 與你自己的（`{home}/AGENTS.md`，也就是本檔）在具體規範或定義上不一致，一律以 working directory（該 repo）的 `CLAUDE.md`/`AGENTS.md` 為準。此優先序不適用於本檔第 2 節「絕對鐵則」——安全限制、Worktree 隔離、權限限制等對所有 repo 一視同仁，任何 repo 的 `CLAUDE.md` 都不能推翻。
